...
- **Docker Limitations**: Due to the absence of a Docker daemon in the current build environment, container execution could not be verified locally. The Docker configurations (`Dockerfile`, `.dockerignore`, build/run commands) are fully prepared for execution on a Docker-enabled machine.
- **Real RAG Test**: A real RAG query against `POST /api/v1/query` utilizing `gemini-3.6-flash` (executed on the local server in lieu of Docker) returned successfully with HTTP 200, querying the in-memory FAISS index.
- **Cost Calculation**: For the 4,359 total tokens processed during the real query, the calculated cost is **$0.0176505**, using the current official API pricing for the `gemini-3.6-flash` model ($1.50/1M input, $7.50/1M output). Full details and formulas are recorded in `RAG_COST_REPORT.md`.
